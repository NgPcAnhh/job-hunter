...
def run_pipeline(sites=None, pages=10, max_jobs_per_page="max", dedup_threshold=0.90, skip_sync=False):
    ...
    for site_name in targets:
        ...
        try:
            ...
            results_summary[site_name] = {"status": "SUCCESS", "jobs": job_count, "duration": duration}
            logger.info(f"✅ Hoàn tất crawl {site_name}: {job_count} jobs vào `jobs_{site_name}` ({duration:.1f}s).")
            
            # Gửi thông báo tiến trình hoàn tất từng trang
            notify_site_progress(site_name, job_count, "SUCCESS", duration)
        except Exception as e:
            duration = time.time() - site_start_time
            logger.error(f"❌ Thất bại khi cào {site_name}: {e}")
            results_summary[site_name] = {"status": "FAILED", "error": str(e), "jobs": 0, "duration": duration}
